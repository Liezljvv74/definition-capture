...
/** Splits on [[…]] while keeping the delimiters, so word links survive. */
const NAME_LINK = /(\[\[[^[\]\n]+\]\])/g;

/**
 * The same rule as `NAME_LINK`, anchored, for checking a whole segment.
 *
...
export function parseRef(text: string): RefToken[] {
  ...
  for (const segment of text.split(NAME_LINK)) {
    if (!segment) continue;
